...
export function CheckoutPage() {
  ...
  const typedItems = items.filter((item): item is (typeof products)[number] =>
    Boolean(item),
  );
  ...
  const submitDetails = (event: FormEvent<HTMLFormElement>) => {
    ...
    const digits = next.phone.replace(/[^0-9۰-۹]/g, "");
    const nextErrors: Partial<Record<keyof Details, string>> = {};
    if (next.name.length < 3) nextErrors.name = t("nameError");
    if (digits.length < 10) nextErrors.phone = t("phoneError");
    if (next.city.length < 2) nextErrors.city = t("cityError");
    if (next.address.length < 8) nextErrors.address = t("addressError");
    if (Object.keys(nextErrors).length) {
      setErrors(nextErrors);
      refs[(Object.keys(nextErrors) as (keyof Details)[])[0]].current?.focus();
      return;
    }
    setErrors({});
    setDetails(next);
  ...
  const finish = () => {
    ...
    placeOrder({
      ...
      total: totalToman,
      usdTotal: totalUsd,
      delivery: { name: details.name, city: details.city },
    });
    addNotice("order");
    clear();
    setComplete(true);
  ...
  const fieldError = (key: keyof Details) =>
    errors[key] && (
      <small id={`checkout-${key}-error`} className="field-error">
        {errors[key]}
      </small>
    );
  ...
  return (
    <section className="checkout-page section">
      ...
      <div className="checkout-grid">
        {!details ? (
          <form className="checkout-form" onSubmit={submitDetails} noValidate>
