  ...
  return (
    <aside className="install-prompt" aria-label={t("pwaPromptTitle")}>
      <div>
        ...
        <small>{t("pwaInstallSteps")}</small>
      </div>
      <div>
        {installEvent && (
          <button className="button light" type="button" onClick={install}>
            {t("installApp")}
          </button>
        )}
        <button
...
export function PwaPanel() {
  ...
  return (
    <section className="profile-panel pwa-panel">
      <span className="eyebrow">{t("pwaEyebrow")}</span>
      <h2>{t("pwaTitle")}</h2>
      <p>{standalone ? t("pwaInstalled") : t("pwaInstallHelp")}</p>
      {!standalone && (
        <small className="pwa-steps">{t("pwaInstallSteps")}</small>
      )}
