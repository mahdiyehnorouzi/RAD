export const fa = {
  navProducts: "آثار",
  navStudio: "طراحی اختصاصی",
  navAbout: "درباره رَد",
  navJournal: "ژورنال",
  bag: "کیسه",
  bagAria: "مشاهده محصولات و کیسه خرید",
  navAria: "ناوبری اصلی",
  openMenu: "باز کردن منو",
  closeMenu: "بستن منو",
  search: "جست‌وجو",
  searchAria: "جست‌وجوی آثار",
  searchPlaceholder: "نام یا ویژگی اثر را جست‌وجو کنید…",
  searchResults: "نتایج جست‌وجو",
  searchEmpty: "اثری با این مشخصات پیدا نشد.",
  clearSearch: "پاک‌کردن جست‌وجو",
  closeSearch: "بستن جست‌وجو",
  closeToast: "بستن پیام",
  toastFavoriteAdded: "به علاقه‌مندی‌ها اضافه شد:",
  toastFavoriteRemoved: "از علاقه‌مندی‌ها حذف شد:",
  toastReviewAdded: "نظر شما ثبت شد.",
  toastCartAdded: "به کیسه اضافه شد:",
  imageUnavailable: "تصویر محصول هنوز ثبت نشده",
  imageNumber: "تصویر",
  orders: "پیگیری سفارش",
  ordersEyebrow: "تاریخچه سفارش",
  ordersTitle: "پیگیری سفارش",
  orderId: "شماره سفارش",
  orderStatus: "وضعیت",
  orderReceived: "درخواست دریافت شد",
  orderProcessing: "در حال آماده‌سازی",
  orderShipped: "ارسال شده",
  orderDelivered: "تحویل داده شده",
  orderProgress: "مراحل سفارش",
  orderDate: "تاریخ ثبت",
  orderItems: "آثار سفارش",
  noOrders: "هنوز سفارشی از مجموعه ثبت نکرده‌اید.",
  noOrdersBody: "پس از خرید اثر آماده، پرداخت، بسته‌بندی و ارسال را اینجا می‌بینید.",
  viewAvailableWorks: "دیدن آثار موجود",
  startCustomOrder: "شروع یک سفارش اختصاصی",
  makingProcessTitle: "مسیر ساخت چگونه است؟",
  trackOrder: "پیگیری سفارش",
  reviewsEyebrow: "نظر خریداران",
  reviewsTitle: "دیدگاه درباره این اثر",
  ratingLabel: "امتیاز شما",
  commentLabel: "نظر شما",
  commentPlaceholder: "تجربه یا برداشت‌تان از این اثر را بنویسید…",
  photoLabel: "افزودن تصویر",
  photoHelp: "یک تصویر JPG، PNG یا WebP تا حجم ۱ مگابایت",
  photoRemove: "حذف تصویر",
  photoError: "تصویر باید JPG، PNG یا WebP و کوچک‌تر از ۱ مگابایت باشد.",
  reviewError: "امتیاز و نظری با حداقل ۳ نویسه وارد کنید.",
  submitReview: "ثبت نظر",
  loginToReview: "برای ثبت نظر وارد حساب شوید.",
  noReviews: "هنوز نظری برای این اثر ثبت نشده است.",
  reviewPhotoAlt: "تصویر ارسالی کاربر برای نظر محصول",
  profile: "پروفایل",
  login: "ورود",
  logout: "خروج از حساب",
  notifications: "اعلان‌ها",
  notificationCenter: "مرکز اعلان",
  markAllRead: "خواندن همه",
  noNotifications: "هنوز اعلانی ندارید.",
  noticeFavorite: "به علاقه‌مندی‌ها اضافه شد:",
  noticeCart: "به کیسه اضافه شد:",
  noticeWelcome: "خوش آمدید؛ پروفایل شما روی این دستگاه فعال شد.",
  noticeOrder: "سفارش نمایشی شما ثبت شد. وضعیت پرداخت و ارسال را در سفارش‌ها ببینید.",
  noticeCommissionApproved: "طرح اختصاصی شما تأیید شد. پیشنهاد قیمت در مسیر ساخت می‌آید.",
  noticeCommissionDeclined: "طرح اختصاصی شما ساخته نمی‌شود. دلیل در مسیر ساخت ثبت شده است.",
  noticeCommissionChange: "هنرمند برای طرح اختصاصی شما یک تغییر مشخص خواسته است.",
  noticeCommissionMessage: "پیام تازه‌ای از کارگاه روی سفارش اختصاصی شماست.",
  home: "خانه رَد",
  logoSubtitle: "گالری آثار یکتا",
  collectionEyebrow: "مجموعه ۰۱",
  studioEyebrow: "رَد × تفاوت زیبا",
  processEyebrow: "سفارش اختصاصی",
  shopEyebrow: "فروشگاه آثار یگانه",
  relatedEyebrow: "آثار پیشنهادی",
  bagEyebrow: "کیسه شما",
  designerEyebrow: "رَد × تفاوت زیبا",
  footerTagline: "هر اثر، یک‌بار.",
  footerStudio: "استودیو",
  footerUnique: "آثار یگانه",
  footerCustom: "سفارش اختصاصی",
  footerRad: "رَد",
  footerStory: "داستان ما",
  footerCommunity: "اجتماع",
  footerInstagram: "اینستاگرام",
  footerNewsletter: "خبرنامه",
  footerCopyright: "© ۱۴۰۵ استودیو رَد — تهران",
  enamad: "نماد اعتماد الکترونیکی",
  heroEyebrow: "آثار یکتای هنری • تهران",
  heroTitle1: "هیچ‌چیز دوبار",
  heroTitleLead: "هیچ‌چیز",
  heroStamp: "دوبار",
  heroTitle2: "ساخته نمی‌شود.",
  heroTitleRest: "ساخته نمی‌شود.",
  heroViewWorks: "مشاهده آثار",
  heroBody:
    "هر اثر رَد یگانه است؛ ساخته‌شده با دست و ردّی که فقط یک‌بار به‌جا می‌ماند.",
  designMine: "طراحی قطعه‌ی من",
  startDesign: "شروع طراحی",
  startCustomDesign: "شروع طراحی اختصاصی",
  viewWorks: "دیدن آثار",
  onlyOne: "تنها یک نسخه",
  artworkLabel: "اثر شماره ۲۷ — تهران، ۱۴۰۵",
  featureHandmade: "ساخته‌شده با دست",
  featureUnique: "تنها یک نسخه",
  featureTehran: "طراحی در تهران",
  featureIran: "ارسال به سراسر ایران",
  collectionTitle: "آثار یگانه",
  collectionBody:
    "نقاشی، پارچه، چوب، سرامیک و اشیای هنری؛ هرکدام شماره‌گذاری‌شده و غیرقابل تکرار.",
  allWorks: "همه‌ی آثار",
  studioTitle1: "خیال شما،",
  studioTitle2: "در قالب یک اثر.",
  studioBody:
    "ایده‌ی قطعه‌ی دلخواه‌تان را بنویسید، تصویر اولیه را با هوش مصنوعی بسازید و همراه هنرمند رَد آن را برای ساخت نهایی کنید.",
  enterStudio: "ورود به استودیوی طراحی",
  philosophy: "فلسفه‌ی رَد",
  philosophyTitle: "ارزش در تکرارناپذیری است.",
  philosophyBody:
    "ما قالب ثابت نداریم. هر اثر از گفت‌وگوی ماده، نگاه و دست هنرمند شکل می‌گیرد. تفاوت‌های کوچک بخشی از هویت اثرند، نه نقص آن. رَد هر اثر را در تیراژ یک ارائه می‌کند.",
  processTitle: "سفارش اختصاصی",
  personalizedWhat:
    "سفارش اختصاصی خرید اثر آماده از فروشگاه نیست. یک قطعه‌ی یک‌به‌یک است که از توصیف شما شروع می‌شود، فقط پس از تأیید هنرمند ساخته می‌شود، و تمام مسیرش به‌صورت زندگی‌نامه ثبت می‌ماند.",
  processLead:
    "این مسیر شش بخش دارد. استودیوی طراحی پنج قدم فرم است؛ ساخت بعد از ارسال، ده مرحله‌ی کارگاه دارد. آنچه در خانه می‌خوانید همان شش بخشی است که باید پیش از شروع بدانید.",
  zundTitle: "زونْد چیست؟",
  zundBody:
    "زونْد (Zünd) کاتر دیجیتال کارگاه است. وقتی طرح تأیید شد و ماده به یک خط برش دقیق نیاز دارد — پارچه، چوب، چاپ، چرم، یا شابلون سرامیک — همان نقشه روی زونْد بریده می‌شود. برش زونْد خودِ اثر نیست؛ قالب یا قطعه‌ی دقیق است. بعد از آن دست هنرمند، و در سرامیک کوره، حرف آخر را می‌زنند.",
  step1Title: "توصیف",
  step1Body: "نوع اثر، اندازه، ماده، کاربرد و حسی را که می‌خواهید در استودیو می‌نویسید.",
  step2Title: "تصویر نیت",
  step2Body: "هوش مصنوعی و عکس‌های مرجع یک تصویر اولیه می‌سازند. این نیت است، نه وعده‌ی شیء نهایی.",
  step3Title: "بازبینی هنرمند",
  step3Body: "طرح به دفتر کوره می‌رود. هنرمند تأیید می‌کند، تغییر مشخص می‌خواهد، بدیل می‌دهد، یا با دلیل رد می‌کند.",
  step4Title: "پیشنهاد و بیعانه",
  step4Body: "پس از تأیید، قیمت، زمان و قواعد می‌آید. ساخت فقط با پذیرش مشخصات و بیعانه شروع می‌شود.",
  step5Title: "ساخت و زونْد",
  step5Body: "کارگاه فرم را می‌سازد. اگر برش دقیق لازم باشد زونْد نقشه را می‌بُرد؛ بعد دست و ماده کار را تمام می‌کنند.",
  step6Title: "رونمایی و ارسال",
  step6Body: "پیش از کوره، پخت، رونمایی، مانده‌حساب و ارسال در همان مسیر ساخت ثبت می‌شود.",
  journalTitle1: "ماده، رنگ و",
  journalTitle2: "حافظه‌ی دست",
  journalBody:
    "قصه‌ی هنرمندان، مواد، رنگ‌ها و زندگی آرام اشیایی که هر روز کنارمان هستند.",
  readJournal: "خواندن ژورنال",
  shopTitle: "همه‌ی آثار",
  shopBody:
    "هر اثر فقط یک‌بار ساخته شده است. موجودی این صفحه یک مجموعه‌ی زنده و دائماً در حال تغییر است.",
  filterAll: "همه",
  filterAvailable: "موجود",
  filterReserved: "رزرو‌شده",
  filterSold: "فروخته‌شده",
  filterVases: "گلدان",
  filterTableware: "ظروف",
  filterSculpture: "مجسمه",
  availabilityFilterAria: "فیلتر موجودی آثار",
  availabilityHeading: "موجودی",
  availableWorks: "اثر در مجموعه",
  emptyTitle: "فعلاً اثری در این گروه نیست.",
  emptyBody:
    "مجموعه‌ی رَد دائماً تغییر می‌کند؛ گروه دیگری را ببینید یا قطعه‌ی خودتان را طراحی کنید.",
  seeAll: "دیدن همه‌ی آثار",
  viewProduct: "مشاهده",
  addFavorite: "افزودن به علاقه‌مندی‌ها",
  removeFavorite: "حذف از علاقه‌مندی‌ها",
  saveFavorite: "ذخیره اثر",
  savedFavorite: "ذخیره‌شده",
  backWorks: "بازگشت به آثار",
  previousPage: "بازگشت به صفحه قبل",
  previousWorks: "آثار قبلی",
  nextWorks: "آثار بعدی",
  uniqueAvailable: "اثر یگانه / موجود",
  addBag: "افزودن به کیسه",
  quickAdd: "افزودن سریع",
  inBag: "در کیسه است ✓",
  soldOut: "فروش رفته",
  reserved: "رزرو شده",
  shipping: "ارسال رایگان و بیمه‌شده در ایران • آماده‌سازی ۲ تا ۴ روز کاری",
  moreWorks: "آثار دیگر",
  emptyBag: "کیسه‌ی شما خالی است.",
  emptyBagBody:
    "هر اثر رَد فقط یک نسخه دارد. مجموعه‌ی تازه را ببینید و اثر خودتان را انتخاب کنید.",
  shoppingBag: "کیسه‌ی خرید",
  clearBag: "خالی‌کردن کیسه",
  uniquePiece: "اثر یگانه • ۱/۱",
  removeBag: "حذف از کیسه",
  orderSummary: "خلاصه سفارش",
  worksSubtotal: "جمع آثار",
  insuredShipping: "ارسال بیمه‌شده",
  free: "رایگان",
  finalTotal: "مبلغ نهایی",
  checkout: "ادامه فرایند خرید",
  checkoutNote: "پرداخت آنلاین در نسخه‌ی بعدی فروشگاه فعال می‌شود.",
  accountEyebrow: "حساب رَد",
  loginTitle: "ورود به فضای شخصی",
  loginBody: "علاقه‌مندی‌ها، اعلان‌ها و مسیر سفارش را در یک‌جا نگه دارید.",
  localAccountNote: "اطلاعات حساب شما به‌صورت امن نگهداری می‌شود.",
  forgotPassword: "رمز عبور را فراموش کرده‌اید؟",
  forgotPasswordTitle: "بازیابی رمز عبور",
  forgotPasswordBody:
    "ایمیل حساب را وارد کنید. اگر حسابی با این ایمیل باشد، کد بازیابی برایتان ارسال می‌شود.",
  sendResetCode: "ارسال کد بازیابی",
  resetCodeLabel: "کد بازیابی",
  resetPasswordTitle: "رمز تازه",
  resetPasswordBody: "کد ارسال‌شده و رمز عبور تازه را وارد کنید.",
  resetPassword: "ثبت رمز تازه",
  resetSent: "اگر این ایمیل ثبت شده باشد، کد بازیابی ارسال شده است.",
  backToLogin: "بازگشت به ورود",
  nameLabel: "نام و نام خانوادگی",
  emailLabel: "ایمیل",
  passwordLabel: "رمز عبور",
  passwordHelp:
    "حداقل ۸ نویسه؛ امکان چسباندن و استفاده از مدیر رمز عبور فعال است.",
  showPassword: "نمایش رمز عبور",
  hidePassword: "پنهان‌کردن رمز عبور",
  loginError: "نام، ایمیل معتبر و رمز عبور حداقل ۸ نویسه‌ای را وارد کنید.",
  loading: "در حال بارگذاری",
  submitting: "در حال ارسال…",
  emailInvalid: "ایمیل معتبر وارد کنید.",
  passwordTooShort: "رمز عبور باید حداقل ۸ نویسه باشد.",
  signupTitle: "ساخت حساب",
  signupBody: "نام، ایمیل و رمز عبور را وارد کنید تا فضای شخصی‌تان ساخته شود.",
  signup: "ساخت حساب",
  needAccount: "حساب ندارید؟ حساب بسازید",
  haveAccount: "حساب دارید؟ وارد شوید",
  accountExists: "این ایمیل قبلاً ثبت شده است. وارد شوید.",
  accountMissing: "حسابی با این ایمیل وجود ندارد. ابتدا حساب بسازید.",
  loginCredentials: "ایمیل یا رمز عبور نادرست است.",
  accountInactive: "این حساب هنوز فعال نشده است.",
  profileEyebrow: "پروفایل شما",
  hello: "سلام،",
  accountOverview: "نمای کلی",
  customOrdersNav: "سفارش‌های اختصاصی",
  accountInfo: "اطلاعات حساب",
  accountNavAria: "ناوبری حساب",
  recentActivity: "فعالیت اخیر",
  noRecentActivity: "هنوز فعالیت اخیری ندارید.",
  collectionPurchase: "خرید از مجموعه",
  customOrderType: "سفارش اختصاصی",
  commissionArtistReview: "در حال بررسی هنرمند",
  customOrdersEyebrow: "سفارش اختصاصی",
  customOrdersTitle: "سفارش‌های اختصاصی شما",
  customOrdersSampleTitle: "نمونه مسیر سفارش اختصاصی",
  customOrdersSampleBody:
    "این پرونده‌ها داستانی و نمایشی‌اند؛ سفارش واقعی شما نیستند و هیچ تغییری در آن‌ها ثبت نمی‌شود.",
  customOrdersBody:
    "پروژه‌های اثر اختصاصی که فرستاده‌اید. مسیر ساخت هر سفارش را جدا باز کنید.",
  viewSamplePath: "دیدن نمونه مسیر",
  makingDemoBanner: "نمونه مسیر سفارش اختصاصی — اطلاعات و دکمه‌ها غیرفعال‌اند.",
  makingPathTitle: "مسیر ساخت اثر من",
  accountInfoEyebrow: "حساب شما",
  accountInfoBody: "نام، ایمیل و نشست این دستگاه را اینجا می‌بینید.",
  favoritesNav: "علاقه‌مندی‌ها",
  favoriteEyebrow: "علاقه‌مندی‌ها",
  savedWorks: "اثر ذخیره‌شده",
  favoriteProfileBody:
    "فهرست شخصی‌تان را ببینید یا آن را با دوستانتان به اشتراک بگذارید.",
  viewFavorites: "دیدن علاقه‌مندی‌ها",
  favoritesTitle: "فهرست آثار محبوب",
  sharedListEyebrow: "فهرست مشترک",
  shareList: "اشتراک‌گذاری فهرست",
  linkCopied: "لینک آماده شد ✓",
  shareFavoritesText: "این آثار یگانه از استودیو رَد را ببینید.",
  emptyFavorites: "هنوز اثری کنار نگذاشته‌اید.",
  emptyFavoritesBody:
    "خرید اثر هنری تصمیم یک‌لحظه‌ای نیست. قلب روی هر اثر، آن را برای دیدن دوباره نگه می‌دارد.",
  emptyFavoritesAction: "دیدن آثار",
  favoritesWaitingTitle: "فهرست در حال چیده‌شدن است.",
  favoritesWaitingBody: "آثار ذخیره‌شده‌تان را می‌آوریم.",
  favoritesWaitingCount: "در حال خواندن فهرست",
  pwaEyebrow: "برنامه رَد",
  pwaTitle: "رَد روی دستگاه شما",
  pwaInstallHelp:
    "از منوی مرورگر، «افزودن به صفحه اصلی» را انتخاب کنید تا رَد مانند یک برنامه اجرا شود.",
  pwaPromptTitle: "رَد را روی صفحه اصلی داشته باشید",
  pwaPromptBody:
    "سایت را مثل یک برنامه باز کنید و سریع‌تر به سفارش‌ها و آثار ذخیره‌شده برسید.",
  pwaInstallSteps:
    "آیفون و آیپد: در Safari روی اشتراک‌گذاری و سپس «افزودن به صفحه اصلی» بزنید. اندروید: منوی Chrome را باز کنید و «نصب برنامه» را انتخاب کنید.",
  installApp: "افزودن به صفحه اصلی",
  dismissInstall: "فعلاً نه",
  pwaInstalled: "نسخه نصب‌شده رَد روی این دستگاه در حال اجراست.",
  enableNotifications: "فعال‌کردن اعلان دستگاه",
  notificationsEnabled: "اعلان دستگاه فعال است",
  notificationReady: "اعلان‌های رَد فعال شد",
  notificationReadyBody: "خبر آثار ذخیره‌شده و سفارش‌ها را اینجا می‌بینید.",
  checkoutEyebrow: "تسویه سفارش",
  checkoutTitle: "جزئیات دریافت اثر",
  checkoutBody:
    "نشانی را وارد کنید. پس از ثبت، مبلغ را به شماره کارت رَد واریز می‌کنید تا درگاه بانکی وصل شود.",
  phoneLabel: "شماره تماس",
  cityLabel: "شهر",
  addressLabel: "نشانی",
  postalCodeLabel: "کد پستی",
  quantityLabel: "تعداد",
  deliveryCost: "هزینه ارسال",
  checkoutError: "همه اطلاعات دریافت را کامل کنید.",
  placingOrder: "در حال ثبت سفارش…",
  requestFailed: "ثبت سفارش انجام نشد. دوباره تلاش کنید.",
  workNoLongerAvailable: "یکی از آثار کیسه دیگر قابل خرید نیست. آن را حذف کنید و دوباره تلاش کنید.",
  nameError: "نام و نام خانوادگی را کامل وارد کنید.",
  phoneError: "شماره تماس معتبر وارد کنید.",
  cityError: "نام شهر را وارد کنید.",
  addressError: "نشانی کامل را وارد کنید.",
  reviewOrder: "بررسی سفارش",
  editDetails: "ویرایش اطلاعات",
  placeDemoOrder: "ادامه و پرداخت",
  orderSuccessTitle: "درخواست شما ثبت شد.",
  orderSuccessBody:
    "سفارش ثبت شد. مبلغ را به شماره کارت اعلام‌شده واریز کنید و پرداخت را تأیید کنید.",
  returnHome: "بازگشت به خانه",
  toman: "تومان",
  designerTitle1: "قطعه‌ای که هنوز",
  designerTitle2: "وجود ندارد.",
  designerBody:
    "اول بگو چه چیزی در ذهن داری. رَد از آن یک کارت ایده می‌سازد، نه تصویر تمام‌شده.",
  designerFreeIdea:
    "کارت ایده نقطه‌ی شروع گفت‌وگو با سازنده است؛ نتیجه از کوره می‌آید.",
  designerFreeTitle: "رَد خودت را شروع کن",
  promptLabel: "چی توی ذهنت داری؟",
  promptPlaceholder:
    "مثلاً یه ماگ می‌خوام شبیه گربه‌م، ولی نمی‌خوام واقعاً شکل گربه باشه.",
  promptHelp:
    "لازم نیست دقیق باشد. حس، رنگ، یا یک خط کج هم کافی است.",
  stopGeneration: "توقف ساخت",
  generate: "ساخت تصویر با هوش مصنوعی",
  generating: "در حال شکل‌دادن به ایده‌ی شما…",
  preview: "پیش‌نمایش قطعه اینجا ساخته می‌شود",
  anotherVersion: "ساخت نسخه‌ی دیگر",
  talkArtist: "گفت‌وگو با هنرمند",
  generatedAlt: "تصویر پیشنهادی اثر سفارشی شما",
  designError: "تصویر ساخته نشد.",
  note1: "تصویر اولیه نیت است، نه شیء.",
  note2: "بازبینی طرح در دفتر کوره انجام می‌شود.",
  note3: "اگر برش دقیق لازم باشد، زونْد نقشهٔ تأییدشده را می‌بُرد.",
  preset1: "رنگ‌های خاکی",
  preset2: "خطوط آزاد",
  preset3: "بافت طبیعی",
  preset4: "هندسه ساده",

  switchToEn: "انگلیسی",
  switchToFa: "فارسی",
  switchToEnAria: "Switch to English",
  switchToFaAria: "تغییر زبان به فارسی",
  tehranYear: "تهران — ۱۴۰۵",
  heroThesis: "یک اثر. یک ردِ دست. یک بار.",
  tehranSlashYear: "تهران / ۱۴۰۵",
  oneOfOne: "۱ / ۱",
  editionMark: "۱/۱",
  evidenceEyebrow: "مدرک ساخت",
  evidenceTitle: "دست، ماده، اثر.",
  evidenceBody: "هر ماده، ریتم و رد خودش را دارد.",
  evidenceMeta: "ویدیو ۰۰:۰۴ — تهران / ماده، فرم، پرداخت، امضا",
  evidenceFilmAlt: "ویدیوی کوتاه یکی از مسیرهای ساخت اثر",
  evidenceCaption: "تهران / ماده، فرم، پرداخت، امضا",
  entryPathsAria: "دو مسیر رَد",
  ownPathIndex: "I — انتخاب",
  ownPathTitle: "آثار آماده",
  ownPathBody: "اثری را که همین حالا در کارگاه تمام شده انتخاب کنید.",
  createPathIndex: "II — خلق",
  createPathTitle: "سفارش اختصاصی",
  createPathBody: "ایده را توصیف کنید و ساخت را با هنرمند جلو ببرید.",
  makingPathPhotoAlt: "گفت‌وگوی مشتری و هنرمند درباره طرح یک کاسه سفالی",
  archiveEyebrow: "آرشیو رَد",
  archiveTitle: "آثار تازه",
  archiveBody: "چند اثر منتخب از مجموعهٔ زنده؛ آرشیو کامل در صفحهٔ آثار است.",
  homeArchiveLink: "دیدن همه آثار و آرشیو",
  studioImagineTitle: "چیزی را که هنوز وجود ندارد، تصور کن.",
  studioImagineBody: "رَد کمک می‌کند آن را ببینی و بسازی.",
  storyLead:
    "رَد از میل به ساختن اشیایی شروع شد که برای هیچ‌کس دیگری ساخته نشده‌اند.",
  storyBodyLong:
    "هر اثر در گفت‌وگوی مستقیم میان نگاه هنرمند، ماده و دست شکل می‌گیرد. تفاوت‌ها نقص نیستند؛ امضای فرآیندند.",
  provenanceEyebrow: "III — منشأ اثر",
  provenanceTitle: "ارزش یک اثر، در مسیر ساخت آن است.",
  certificateEyebrow: "شناسنامه اثر",
  certificateTitle: "هر رَد یک زندگی دارد",
  certificateArtist: "ساخته‌شده توسط هنرمند",
  certificateMaterial: "ثبت متریال و مراحل ساخت",
  certificateNumber: "شماره یکتا و نشان ۱/۱",
  certificatePhotoAlt: "اثر تمام‌شده در فضای کارگاه",
  homeProcessEyebrow: "مسیر سفارش",
  homeProcessLead: "از توصیف تا ساخت؛ فقط اگر هنرمند بپذیرد.",
  homeProcessIndex1: "۰۱",
  homeProcessIndex2: "۰۲",
  homeProcessIndex3: "۰۳",
  homeProcessStep1: "ایده‌ات را توصیف کن",
  homeProcessStep2: "طرح و پیشنهاد هنرمند را ببین",
  homeProcessStep3: "مسیر ساخت را دنبال کن",
  homeProcessBody1: "نوع اثر، ماده، اندازه و حسی را که می‌خواهید می‌نویسید. این نقطه‌ی شروع است، نه طرح نهایی.",
  homeProcessBody2: "هنرمند طرح را می‌خواند، پیشنهاد می‌دهد، و ساخت فقط اگر بپذیرد آغاز می‌شود.",
  homeProcessBody3: "هر مرحله ثبت می‌شود. مسیر ساخت همان زندگی‌نامه‌ی اثر است.",
  homeProcessDetails: "دیدن جزئیات مسیر ساخت",
  provenanceMadeTitle: "ساخت در تهران",
  provenanceMadeBody:
    "هنرمند، متریال و تمام مراحل ساخت هر اثر در شناسنامه آن ثبت می‌شود.",
  provenanceHandTitle: "ردِ دست حفظ می‌شود",
  provenanceHandBody:
    "تفاوت‌های طبیعی هر ماده پنهان نمی‌شوند؛ همان‌ها بخشی از هویت اثرند.",
  provenanceNumberTitle: "شماره و شناسنامه",
  provenanceNumberBody: "هر اثر با شماره، متریال، سال و نشان ۱/۱ ثبت می‌شود.",
  ordersEntryEyebrow: "سفارش‌های شما",
  ordersEntryTitle: "خرید آماده را از ساخت اختصاصی جدا دنبال کنید.",
  ordersEntryBody:
    "اثر آماده فقط پرداخت، بسته‌بندی و ارسال دارد. طرح اختصاصی بازبینی هنرمند و مسیر ساخت خودش را دارد.",
  ordersEntryShop: "پیگیری خرید آماده",
  viewOrders: "دیدن سفارش‌ها",
  artworkCategoriesHeading: "دسته‌بندی آثار",
  swipeToSeeMore: "برای دیدن بیشتر بکشید",
  filterCategoriesAria: "فیلتر دسته‌بندی آثار",
  radDelivery: "ارسال آثار رَد",
  beforeYouBuy: "پیش از خرید بدانید",
  faqDamageQ: "اگر اثر در ارسال آسیب ببیند؟",
  faqDamageA:
    "تمام آثار بیمه‌اند. آسیب را تا ۲۴ ساعت با عکس اعلام کنید؛ رَد مسئول پیگیری و جبران است.",
  faqPackQ: "بسته‌بندی چگونه است؟",
  faqPackA:
    "هر اثر در جعبه دولایه، با محافظ متناسب با فرم و شناسنامه امضاشده ارسال می‌شود.",
  faqTimeQ: "زمان و محدوده ارسال؟",
  faqTimeA:
    "تهران ۲ تا ۴ روز کاری و شهرستان ۴ تا ۸ روز کاری؛ ارسال بیمه‌شده رایگان است.",
  faqReturnQ: "رنگ، متریال و مرجوعی",
  faqReturnA:
    "نور نمایشگر می‌تواند رنگ و بافت را کمی تغییر دهد. آثار آماده تا ۴۸ ساعت امکان درخواست بازگشت دارند؛ سفارش شخصی مرجوع نمی‌شود.",
  guestCollector: "کاربر رَد",
  tehranCity: "تهران",
  designerConnect: "رَد شما را به هنرمند و مسیر ساخت مناسب متصل می‌کند.",
  stepOne: "مرحله اول",
  stepTwo: "مرحله دوم",
  chooseArtworkType: "چه نوع اثری می‌خواهید؟",
  promptChooseCategory: "ابتدا نوع اثر را انتخاب کنید…",
  suggestedWords: "واژه‌های پیشنهادی",
  designDirection: "جهت طراحی",
  imagineIt: "تصورش کن ←",
  chooseCategoryToBegin: "برای شروع، نوع اثر را انتخاب کنید.",
  briefDetails: "جزئیات {name}",
  titleAccount: "حساب کاربری",
  titleFavorites: "علاقه‌مندی‌ها",
  titleCart: "کیسه خرید",
  titleCheckout: "ثبت سفارش",
  titleProducts: "آثار",
  titleStudio: "طراحی اختصاصی",
  titleMaking: "مسیر ساخت اثر من",
  titleCustomOrders: "سفارش‌های اختصاصی",
  titleNotifications: "اعلان‌ها",
  titleAccountInfo: "اطلاعات حساب",
  titleOrders: "پیگیری سفارش",
  titleWorkshop: "کارگاه هنرمند",
  titleDifferences: "موزه تفاوت‌ها",
  makingEyebrow: "کارگاه مشتری",
  makingNav: "مسیر ساخت",
  makingTitle: "مسیر ساخت اثر من",
  makingBody:
    "این گفت‌وگوی آزاد نیست؛ زندگی‌نامه ساخت است. هر سفارش همیشه می‌گوید قطعه در کدام مرحله است، چه کسی باید عمل کند، و بعد از آن چه می‌شود.",
  makingAfterSend:
    "با ارسال، طرح برای هنرمند در دفتر کوره ثبت می‌شود. پاسخ — تأیید، تغییر، یا رد — همین‌جا در مسیر ساخت و در اعلان‌ها می‌آید.",
  makingEmpty: "هنوز ساخت اثری را شروع نکرده‌اید.",
  makingEmptyBody: "ایده‌تان را تعریف کنید تا هنرمند آن را بررسی کند.",
  makingBack: "بازگشت به سفارش‌های اختصاصی",
  makingMissing: "این مسیر ساخت پیدا نشد.",
  makingSubmit: "ارسال طرح برای ساخت",
  makingUseLabel: "کاربرد مورد نظر",
  makingUsePlaceholder: "مثلاً سفره روزمره، دیوار ورودی، نگاه روی طاقچه…",
  makingUseHelp: "هنرمند کاربرد را در امکان ساخت و ضخامت فرم می‌خواند.",
  workshopEyebrow: "کارگاه هنرمند",
  workshopTitle: "کارگاه هنرمند",
  workshopBody:
    "پذیرش طرح و پیام با مشتری در دفتر کوره (پنل مدیریت) انجام می‌شود. این صفحه ادامه‌ی ساخت پس از تأیید است: پیشنهاد، عکس کارگاه، کوره و ارسال.",
  workshopNeeds: "نیاز به اقدام من",
  workshopWaiting: "در انتظار مشتری",
  workshopStages: "مرحله تولید",
  workshopQuiet: "در این ستون کاری نیست.",
  workshopBack: "بازگشت به کارگاه",
  memoryLabel: "یک خاطره، مکان یا آیین (اختیاری)",
  memoryPlaceholder: "مثلاً صدای حیاط مادربزرگ، کوچه‌ای در تهران، حس دلتنگی…",
  memoryHelp: "رَد کیفیت احساسی را به وزن، بافت و رنگ ترجمه می‌کند.",
  surpriseLegend: "چقدر اجازه غافلگیری می‌دهید؟",
  differenceEyebrow: "تفاوت زیبا",
  differenceTitle: "پرتره تفاوت",
  differenceBody:
    "انگشت را بکش: ایده، دست، ماده، رَد. فاصله‌ها پنهان نمی‌شوند.",
  differenceTrail: "مسیر چهار نسخه",
  differenceReveal: "نمایش خصوصی اثر",
  differenceRevealing: "در حال آشکار شدن…",
  differenceCertificate: "شناسنامه مسیر ساخت",
  differenceCertificateBody:
    "این نشان، رد کامل خلق اثر را نگه می‌دارد؛ از خیال تا آتش.",
  differenceOpen: "دیدن پرتره کامل",
  fingerprintBatch: "دسته ماده",
  fingerprintSurface: "رفتار سطح",
  fingerprintFiring: "شرایط پخت",
  fingerprintMark: "نامنظمی مرئی",
  museumEyebrow: "موزه تفاوت‌ها",
  museumTitle: "موزه تفاوت‌ها",
  museumBody:
    "آثاری که فاصله میان نیت، نگاه و ماده را پنهان نکرده‌اند. هر تفاوت امضای فرآیند است.",
  museumBack: "بازگشت به موزه",
  museumMissing: "این پرتره هنوز در آرشیو نیست.",
  homeDifferenceTitle: "فاصله خیال تا واقعیت",
  impossibleEyebrow: "آرشیو سفارش‌های ناممکن",
  impossibleTitle: "چیزی را سفارش دهید که گفتنش سخت است.",
  impossibleBody:
    "کاسه‌ای که حس دلتنگی داشته باشد. گلدانی از یک کوچه فراموش‌شده. هنرمند انتخاب می‌کند چگونه ناممکن را مذاکره کند.",
  brandName: "رَد",
  processStep1: "۰۱/۰۶",
  processStep2: "۰۲/۰۶",
  processStep3: "۰۳/۰۶",
  processStep4: "۰۴/۰۶",
  processStep5: "۰۵/۰۶",
  processStep6: "۰۶/۰۶",
  orderStage1: "تأیید طرح",
  orderStage2: "انتخاب هنرمند",
  orderStage3: "انتخاب متریال",
  orderStage4: "نمونه اولیه",
  orderStage5: "ساخت",
  orderStage6: "پرداخت نهایی",
  orderStage7: "کنترل کیفیت",
  orderStage8: "امضا و شماره ۱/۱",
  orderStage9: "بسته‌بندی",
  orderStage10: "ارسال",
  shopStagePaymentPending: "در انتظار پرداخت",
  shopStageConfirmed: "سفارش ثبت شد",
  shopStagePacking: "در حال بسته‌بندی",
  shopStageShipped: "تحویل به پست",
  shopStageDelivered: "تحویل داده شد",
  shopStageCancelled: "لغوشده",
  shopStageReturned: "مرجوع‌شده",
  ordersShopNote: "این صفحه خرید آثار آماده از مجموعه است.",
  artworkNumber: "شماره اثر رَد",
  shippingAddress: "نشانی ارسال",
  trackingCode: "کد پیگیری پست",
  estimatedDelivery: "زمان تقریبی تحویل",
  orderTotal: "مبلغ کل",
  orderMissing: "این سفارش پیدا نشد.",
  viewOrder: "جزئیات سفارش",
  confirmDemoPayment: "تأیید پرداخت",
  demoPaymentHint: "درگاه واقعی هنوز متصل نیست. این دکمه فقط سفارش نمایشی را ثبت‌شده می‌کند.",
  manualPaymentHint:
    "درگاه بانکی به‌زودی وصل می‌شود. فعلاً مبلغ سفارش را به شماره کارت زیر واریز کنید.",
  manualPaymentCardLabel: "شماره کارت",
  manualPaymentCopyCard: "کپی شماره کارت",
  manualPaymentCardCopied: "شماره کارت کپی شد.",
  manualPaymentAmount: "مبلغ قابل پرداخت: {amount}",
  manualPaymentConfirmNote:
    "پس از واریز، تصویر رسید را بارگذاری کنید تا تیم رَد پرداخت را بررسی و تأیید کند.",
  confirmManualPayment: "ارسال رسید و ثبت پرداخت",
  receiptUploadLabel: "تصویر رسید بانکی",
  receiptPreviewAlt: "پیش‌نمایش رسید پرداخت",
  receiptImageError: "فقط تصویر JPEG، PNG یا WebP تا ۱ مگابایت مجاز است.",
  receiptRequired: "برای ادامه، تصویر رسید را انتخاب کنید.",
  receiptAwaitingReview:
    "رسید شما دریافت شد و در انتظار تأیید رَد است. به‌محض تأیید، سفارش ادامه پیدا می‌کند.",
  gatewayPaymentHint: "برای تکمیل خرید به درگاه امن پرداخت منتقل می‌شوید.",
  continueToGateway: "ورود به درگاه پرداخت",
  cancelDemoOrder: "لغو سفارش",
  copyTracking: "کپی کد پیگیری",
  trackingCopied: "کد پیگیری کپی شد.",
  leaveReview: "ثبت نظر برای این اثر",
  backToOrders: "بازگشت به سفارش‌ها",
  nextActionBrowse: "دیدن آثار آماده",
  waitingPacking: "اثر آماده است و به‌زودی بسته‌بندی می‌شود.",
  waitingShip: "بسته‌بندی در جریان است. کد پستی پس از تحویل به پست اینجا می‌آید.",
  waitingDelivery: "بسته به پست تحویل شده است.",
  orderComplete: "سفارش به دست شما رسیده است.",
  makingCustomNote: "این صفحه سفارش‌های اختصاصی ارسال‌شده است — نه خرید اثر آماده.",
  currentStage: "مرحله فعلی",
  youAreHere: "شما اینجایید",
  stageOf: "مرحله {current} از {total}",
  nextStageLabel: "مرحله بعد",
  designerStepType: "نوع اثر",
  designerStepDetails: "جزئیات",
  designerStepIdea: "توصیف",
  designerStepImages: "تصاویر",
  designerStepSpark: "ذهن",
  designerStepFreedom: "آزادی",
  designerStepSend: "ارسال",
  designerBack: "قبلی",
  designerNext: "بعدی",
  designerSkipImages: "بدون تصویر ادامه بده",
  designerAddImages: "افزودن تصویر مرجع",
  designerImagesHelp: "عکس الهام، طرح دستی یا نمونه رنگ — تا چهار تصویر.",
  designerYourImages: "تصاویر شما",
  designerRemoveImage: "حذف تصویر",
  designerImageError: "فقط JPG، PNG یا WebP تا ۲ مگابایت.",
  designerSparkTitle: "رَد خودت را شروع کن",
  designerSparkLead: "چی توی ذهنت داری؟",
  designerSparkHelp:
    "لازم نیست کامل باشد. یک جمله، یک خط کج، یک صدا یا دو رنگ کافی است.",
  designerSparkPlaceholder:
    "مثلاً یه ماگ می‌خوام شبیه گربه‌م، ولی نمی‌خوام واقعاً شکل گربه باشه.",
  designerWrite: "نوشتن",
  designerDraw: "کشیدن",
  designerVoice: "صدا",
  designerPhoto: "عکس",
  designerColors: "رنگ‌ها",
  designerFeeling: "حس",
  designerDrawHint: "با انگشت یا ماوس یک خط بد بکش. خط بد بهتر است.",
  designerDrawClear: "پاک کردن طرح",
  designerVoiceStart: "شروع ضبط",
  designerVoiceStop: "توقف",
  designerVoiceRecorded: "یادداشت صوتی ثبت شد",
  designerVoiceNeedMic: "برای ضبط صدا اجازه میکروفون لازم است.",
  designerColorLimit: "تا سه رنگ",
  designerFreedomTitle: "چقدر دست رَد باز باشد؟",
  designerFreedomLegend: "آزادی سازنده",
  designerFreedomLow: "نزدیک به چیزی که فرستادم",
  designerFreedomHigh: "غافلگیرم کن",
  designerFreedomShare: "آزادی رَد من: {value}",
  ideaCardLabel: "RAD IDEA",
  ideaCardMood: "حس",
  ideaCardForm: "فرم",
  ideaCardColors: "رنگ",
  ideaCardReference: "مرجع",
  ideaCardFreedom: "آزادی داده‌شده به سازنده",
  ideaCardDisclaimer: "این تصویر نتیجه‌ی نهایی نیست؛ نقطه‌ی شروع ماست.",
  ideaCardEmpty: "هنوز چیزی در کارت نیست.",
  ideaCardHasPhoto: "تصویر بارگذاری‌شده",
  ideaCardHasSketch: "طرح دستی",
  ideaCardHasVoice: "یادداشت صوتی",
  ideaCardHasWriting: "نوشته",
  titlePassport: "گذرنامه رَد",
  footerPassport: "گذرنامه‌ها",
  passportEyebrow: "گذرنامه اثر",
  passportIndexEyebrow: "آرشیو زندگی‌ها",
  passportIndexTitle: "هر رَد یک زندگی دارد",
  passportIndexBody:
    "وقتی اثری ساخته می‌شود، صفحه‌ای برای مسیر، ماده، دست و جایی که حالا هست باقی می‌ماند.",
  passportOnce: "این رَد فقط یک‌بار ساخته شده.",
  passportOnceBody: "اینجا می‌توانی مسیرش را ببینی.",
  passportLedger: "شناسنامه رَد",
  passportMaker: "سازنده",
  passportDate: "تاریخ ساخت",
  passportClay: "گل / ماده",
  passportGlaze: "لعاب / رنگ",
  passportDimensions: "ابعاد",
  passportFiring: "پخت",
  passportInspiration: "الهام اول",
  passportSketch: "اولین طرح",
  passportConstruction: "عکس‌های حین ساخت",
  passportChanges: "تغییرهای غیرمنتظره",
  passportFinal: "عکس‌های نهایی",
  passportOwner: "صاحب",
  passportCity: "شهر",
  passportCare: "نگهداری",
  passportWhere: "این رَد الان کجاست؟",
  passportWhereEyebrow: "مسیر بعد از ساخت",
  passportCopyLink: "کپی پیوند گذرنامه",
  passportCopied: "پیوند کپی شد.",
  passportViewWork: "دیدن اثر در مجموعه",
  passportBack: "همه گذرنامه‌ها",
  passportMissing: "این گذرنامه هنوز نوشته نشده.",
  pdpPassportLink: "گذرنامه این رَد",
  pdpQrLink: "کد QR اثر",
  productQrEyebrow: "برچسب اثر",
  productQrTitle: "کد QR این اثر",
  productQrBody: "این کد را چاپ کنید و روی اثر یا بسته‌بندی بگذارید. با اسکن، صفحهٔ جزئیات اثر باز می‌شود.",
  productQrScanTarget: "مقصد اسکن",
  productQrPrint: "چاپ برچسب",
  productQrCopyLink: "کپی پیوند اثر",
  productQrCopied: "پیوند کپی شد.",
  productQrViewProduct: "دیدن صفحهٔ اثر",
  productQrMissing: "اثر پیدا نشد.",
  beforeRadTitle: "رَد قبل از رَد",
  beforeRadBody:
    "انگشت یا ماوس را بکش تا ایده به دست، ماده و خودِ رَد تبدیل شود.",
  beforeRadHint: "بکش",
  beforeRadIdea: "ایده",
  beforeRadHand: "دست",
  beforeRadMaterial: "ماده",
  beforeRadRad: "رَد",
  liveEyebrow: "در حال ساخته شدن",
  liveHeadline: "رَد {code} دارد ساخته می‌شود.",
  liveNow: "الآن",
  liveDaysAgo: "{days} روز از شروعش گذشته.",
  liveNotes: "یادداشت کارگاه",
  todayWorkshop: "امروز در کارگاه",
  archiveEyebrowFull: "آرشیو رَد",
  archiveTitleFull: "تمام رَدهایی که تا امروز وجود داشته‌اند",
  archiveBodyFull:
    "فروخته‌شده‌ها ناپدید نمی‌شوند. به آرشیو می‌روند و داستان‌شان می‌ماند.",
  archiveMaking: "دارد ساخته می‌شود",
  archiveSoldMark: "SOLD · ۱/۱",
  archiveInStudio: "در کارگاه",
  archiveNeverAgain: "این رَد دیگر تکرار نمی‌شود.",
  archiveBack: "بازگشت به آرشیو",
  sameFeeling: "چیزی با همین حس می‌خواهی؟",
  familyEyebrow: "خانواده‌ی این رَد",
  familyBody: "یگانه‌اند. دی‌ان‌ای مشترک دارند، نه طرح یکسان.",
  marksTitle: "ردهای دست",
  marksBody:
    "نقص‌های ساخت را پنهان نکردیم. روی نقطه بزن و ببین از کجا آمده‌اند.",
  shapeEyebrow: "کشف",
  shapeTitle: "رَد من چه شکلیه؟",
  shapeBody: "پنج سؤال کوتاه. بعد سه رَد واقعی از آرشیو.",
  shapeResult: "تو احتمالاً این رَدی:",
  shapeAgain: "از اول",
  myCollectionTitle: "مجموعه‌ی من",
  myRadsTitle: "رَدهای من",
  myRadsEyebrow: "مجموعه‌ی من",
  myRadsBody: "رَدهایی که مال تو شده‌اند. سفارش نیستند؛ مجموعه‌اند.",
  transferLine: "این رَد از {from} به {to} رسید — {when}.",
  titleArchive: "آرشیو رَد",
  titleNow: "الآن در کارگاه",
  titleShape: "رَد من چه شکلیه؟",
  footerArchive: "آرشیو",
  footerShape: "رَد من چه شکلیه؟",
  footerNow: "امروز در کارگاه",
} as const;

export type MessageKey = keyof typeof fa;
