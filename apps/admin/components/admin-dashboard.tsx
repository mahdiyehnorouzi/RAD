...
export function AdminDashboard() {
  ...
  return <div className="admin-shell">
    ...
    <main className="admin-main">
      <header className="admin-topbar"><button className="menu-button" type="button" onClick={() => setMobileNav(true)} aria-label="باز کردن منو"><Menu /></button><div><span className="context-label">رَد / عملیات استودیو</span><h1>{navItems.find((item) => item.id === section)?.label}</h1></div><div className="topbar-status"><span className="live-dot" />پایگاه داده Postgres</div></header>
    ...
    {deleteTarget && <ConfirmDialog title={`حذف «${deleteTarget.name}»؟`} description="این محصول از پایگاه داده حذف می‌شود و دیگر در فروشگاه دیده نخواهد شد." onCancel={() => setDeleteTarget(null)} onConfirm={async () => { try { await workspace.deleteProduct(deleteTarget.id); setDeleteTarget(null); announce("محصول حذف شد."); } catch (error) { announce((error as Error).message); } }} />}
    {inviteOpen && <InviteDialog onClose={() => setInviteOpen(false)} onInvite={async (member) => { try { await workspace.inviteMember(member); setInviteOpen(false); announce("عضو جدید در پایگاه داده ثبت شد."); } catch (error) { announce((error as Error).message); } }} />}
...
function ProductDialog({ product, onClose, onSave }: { product: AdminProduct | null; onClose: () => void; onSave: (product: AdminProduct) => void | Promise<void> }) {
  ...
  const addImage = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      setDraft((item) => ({ ...item, images: [...item.images, String(reader.result)] }));
      setImageError("");
    };
...
function ConfirmDialog({ title, description, onCancel, onConfirm }: { title: string; description: string; onCancel: () => void; onConfirm: () => void | Promise<void> }) { return <DialogShell title={title} description={description} onClose={onCancel} compact><div className="dialog-actions"><button className="secondary-action" type="button" onClick={onCancel}>انصراف</button><button className="danger-action" type="button" onClick={onConfirm}>حذف محصول</button></div></DialogShell>; }
