...
Product cards use a specimen-plate composition: a quiet monochrome context layer sits behind one crisp colour window, with the `RĀD / NNN` and `1 / 1` identifiers attached as the image label. The effect recalls an archive contact print, not a floating ecommerce tile; metadata remains visible below and cards keep flat edges with no decorative shadow. Desktop and mobile PDPs use the same stacked storehouse composition: one generous framed artwork stage comes first, followed by the acquisition ledger. On desktop the ledger becomes a compact three-column strip; on mobile it returns to a single reading column. The featured work is a transparent, tactile graphic cutout anchored lower-right and breaking past its color field without touching the display name. The display name uses kiln green for reliable contrast. Three smaller category illustrations travel on one continuous, evenly phased orbit around the display name—ceramic bowls and vessels for pottery, or the established tool and material symbols for other categories—and repeat as material markers in the detail ledger. The stage's unused top corner may carry quiet category and maker context on larger screens, but mobile keeps that space clear. The image selector changes the still product view without auto-advancing or moving the product itself; reduced-motion mode also stops the orbit.
