...
Product cards use a specimen-plate composition: a quiet monochrome context layer sits behind one crisp colour window, with the `RĀD / NNN` and `1 / 1` identifiers attached as the image label. The effect recalls an archive contact print, not a floating ecommerce tile; metadata remains visible below and cards keep flat edges with no decorative shadow. Desktop PDPs use a two-part storehouse composition: the artwork and clipped display name remain static on the left, while the acquisition ledger remains static on the right. The featured work is a transparent, tactile graphic cutout that breaks past its color field. Three smaller category illustrations travel on one continuous, evenly phased orbit around the display name—ceramic bowls and vessels for pottery, or the established tool and material symbols for other categories—and repeat as material markers in the detail ledger. The image selector changes the still product view without auto-advancing or moving the product itself; reduced-motion mode also stops the orbit.
