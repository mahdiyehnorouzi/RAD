...
- Product media: `components/product/product-media.tsx`; products may expose multiple images, while missing or explicitly empty image lists render the fantasy fallback owned by the product category.
- Sold media state: `components/product/product-media.tsx`; every `ProductMedia` rendering of a sold work carries the same localized, non-interactive sold badge.
